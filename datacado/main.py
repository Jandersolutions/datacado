...
def generate_sales_data(num_records, mes, ano, num_cpf, num_op):
    ...
    for _ in range(num_records):
        ...
        preco = product_dict[produto]
        quantidade = random.randint(1, 10)
        pdv = random.randint(1, 21)
        operadora = random.choice(operators)
        operacao = random.choice(operations)
        total = round(preco * quantidade, 2)
        dataset.append([cpf, hora, data, produto, preco, quantidade, pdv, operadora, operacao, total])

    # Escreve os registros no arquivo CSV
    filename = f"venda_{mes:02d}-{ano:04d}.csv"
